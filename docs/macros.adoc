...
Occasionally, you may require rich components, even within your rich text. This is enabled via a component type called Macros. Macros follow the same pattern as we have seen for other components

== FactBox macro

In your Enonic project, a `FactBox` macro schema is pre-defined.
It is located in `src/main/resources/cms/macros/factbox/factbox.yaml`
If you take a closer look at the Léa Seydough bio, you can see how the [factbox] macro is used.
...
. *Sweet!*
+
You should now see http://localhost:3000/persons/lea-seydough[Lea Seydough] updated with the `FactBox` macro:
