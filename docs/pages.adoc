...
----
+
. **Verify that pages are no longer rendering json body** by visiting your site on `http://localhost:3000`.
+
...
. Verify Enonic app (not the Next.js frontend) has a *page component descriptor* at:
+
...
+
*Redeploy the Enonic app* to register the page component if it didn't have it (happens automatically if you started `enonic dev`).
+
...
From the edit view, open the Context Panel by clicking the `Show Context Panel` icon in the top right corner, and select `Page` in the widget dropdown at the top of the panel.
+
You should now see the following:
+
image:page-setup.png[title="Selecting the page component in the page view",width=1440px]
+
In the `Inspect` tab of the panel, select the `Main` component we created earlier from the `Template` dropdown (it shows `Automatic` until then).
+
...
To put something on the page, we need a component editors can drop into the region.
Let's create a simple `Paragraph` part that holds rich text.

NOTE: Deeper dive into parts and components will be covered in the next chapter.

. Verify Enonic app (not the Next.js frontend) has **the `paragraph` part**:
+
...
----
+
NOTE: The link:https://developer.enonic.com/docs/xp/stable/cms/schemas/input-types/htmlarea[HtmlArea input] gives editors a rich-text editor with formatting, links, images and macros.
+
**Redeploy the Enonic app** if it didn't have it to register the part (happens automatically if you started `enonic dev`).
+
...
----
+
NOTE: `richTextQuery` generates the query fragment needed for the adapter to resolve link, image and macro metadata within the rich text field of the part config form.
+
. **Add the part to your page**
+
Back in Content Studio, drag `Part` from the `Insert` tab on the right hand side into the region, then select `Paragraph` in the part dropdown of the `Inspect` tab.
Use the link:https://developer.enonic.com/docs/content-studio/stable/editor/component-types#htmlarea[rich-text editor] to write and format text, as well as insert images or macros, and click `Apply`.
+
image:paragraph-component.png[title="Next.js-rendered paragraph part with an image",width=1440px]
+
. You may also click the `Preview` button in Content Studio for a full screen version.

== Task: Publish page

Your new page is looking good.
However, if you visit `http://localhost:3000[http://localhost:3000^]` in an incognito window or different browser - you'll get a "404 page not found" error.
This is because preview mode is off in incognito window and only published changes are visible from the front end.
...
. **Publish the page** by clicking `Mark as ready`, and then `Publish now` in the Publishing Wizard.
+
image:publish.png[title="Publishing wizard",width=810px]
+
. **Verify that your page is live** by visiting `http://localhost:3000[http://localhost:3000^]` in incognito window once again.
