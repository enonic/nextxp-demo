...
+
. Next we need to tell Next.XP where to find the frontend server:
+
...
+
NOTE: We are overriding default configuration, so it will get applied automatically to all sites, that didn't specify otherwise.
+
