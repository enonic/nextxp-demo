...
Enonic provides a highly customizable component type called link:https://developer.enonic.com/docs/xp/stable/components/components#part[parts]. Similar to page components parts must be defined in both Enonic and Next.js in order to fully work.
...
+
Back in Content Studio, edit the `/hmdb/persons` folder item. Create the page and place the new part into the region (`right-click -> Insert`, or drag-n-drop from the `Insert panel`` on the right hand side).
+
