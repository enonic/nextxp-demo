...
Enonic provides a highly customizable component type called link:https://developer.enonic.com/docs/xp/stable/cms/pages/parts[parts^] and you actually have already created one called `Paragraph` in the previous chapter!
Similar to page components, parts must be defined in both Enonic and Next.js in order to fully work.


== Task: Add part to Enonic
We'll start off by adding a simple component that lists child items in the tree structure.

. Verify Enonic app (not the Next.js frontend) has **the `child-list` part descriptor** at:
+
.src/main/resources/cms/parts/child-list/child-list.yaml
[source,yaml]
----
kind: "Part"
title: "Child list"
description: "Lists children of current item"
form:
  - type: "ComboBox"
    name: "sorting"
    label: "Sort order"
...
+
**Build and redeploy your Enonic app** once more if it didn't have it to make the new part available in Content Studio (happens automatically if you started `enonic dev`).
+
. **Add parts to the page**
+
Back in Content Studio, edit the `/hmdb/persons` folder item.
Open the Context Panel (`Show Context Panel` in the top right corner), select `Page` in the widget dropdown and choose `Main` in the Template dropdown to turn the folder into a page.
Drag `Part` from the `Insert` tab into the region, then select `Child list` in the part dropdown of the `Inspect` tab.
+
...
+
.src/main/resources/cms/parts/heading/heading.yaml
[source,yaml]
----
kind: "Part"
title: "Heading"
description: "Demo heading"
form:
- type: "TextLine"
  name: "heading"
...
----
+
**Redeploy** the Enonic app if it didn't have it and add the part to your page (happens automatically if you started `enonic dev`).
+
...
----
+
You should now have three configurable parts to play with.
