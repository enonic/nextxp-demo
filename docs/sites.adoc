...
. **Add your app** to the site - by selecting it in the `Applications` form field and clicking `Apply`.
+
...
Return back to the Content Studio tab, and follow these steps:
...
+
. Wrap up by **publishing the changes**. Select the site (root item) in the tree structure, and click `Mark as ready` in the top right. This marks the item as ready and opens the Publishing Wizard.
+
image:content-studio-mark-as-ready.png[title="Mark as ready", width=1024px]
+
. To **publish the entire new tree structure** toggle `Include child items`, click `Apply` in the wizard header, then click `Publish now`.
+
...
In GraphQL queries, you may use a so-called "site context".
This is achieved by adding another parameter to the guillotine query called `siteKey`.

. Specify `siteKey` parameter
+
Back in Query Playground, add the `siteKey` parameter to your previous query, and set it to the path of your site:
+
[source,graphql]
----
query($path:ID!){
  guillotine(project: "intro", siteKey: "/hmdb") {
    ...
  }
}
----
...
----
{
    "path": "${site}/movies/s7ven"
}
----

NOTE: Here, `${site}` will resolve to the path of the site and expand - in this case the full path will be expanded to `/hmdb/movies/s7ven`.
