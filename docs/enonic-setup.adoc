...
In this chapter you will create a local Enonic developer environment, create and deploy an Enonic app.
...
* and link it to the `nxptutorial` sandbox

. *Deploy the application*
+
To build and run your application, execute these commands (the CLI placed the project in a folder named after the last part of the app name):
...
NOTE: `Intro` should be in the list, together with other applications that were automatically installed when you created the sandbox.

Click btn:[Log In as Guest] to open the XP admin dashboard. You should now see the following:

.The XP admin dashboard
image::xp-dashboard.png[The XP admin dashboard, 1085w]

You are now on the admin dashboard.
The center menu (aka `XP menu`) enables navigation between admin tools.
Admin tools may be injected as extensions via applications.
...
The first time your application was deployed to the sandbox, it automatically created a content project called `Intro Project` (id `intro`) and populated it with content. Since this is the only available project at the moment, Content Studio will automatically open it, revealing the list of sample content items.
