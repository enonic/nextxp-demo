...
For every content project in your sandbox, you will automatically get an API. By clicking `Query Playground` in the Content Studio menu, you should see the following:

image:content-studio-guillotine.png[title="Browsing the Guillotine API using Query Playground",width=1440px]

[NOTE]
====
The API can also be called directly at http://localhost:8080/api/com.enonic.app.guillotine:graphql[http://localhost:8080/api/com.enonic.app.guillotine:graphql^].
Direct calls must pass the `branch` argument (`draft` or `master`) to the `guillotine()` field, e.g. `guillotine(project: "intro", branch: "master")`.
The playground adds it for you, based on its Draft/Master toggle.
====
...
----
+
NOTE: `intro` is the name of the Content Studio project. `tutorial.nxp` refers to the name of the application you created in the previous chapter. `movie` is a specific content type within the app.
+
. **View the Result**
+
...
+
NOTE: The `path` field in the JSON corresponds to the `$path` in the query string above:
+
[source,JSON]
----
{
...
For example, you may introspect the fields that are exclusive to the `tutorial.nxp:movie` content type like this:
...
+
Lets get content for a movie, the referenced _image_ items `media:image` and _person_ items `tutorial.nxp:person` in a single query:
+
...
----
+
Notice how `imageUrl` is requested with parameters of its own?
Type introspections can expose certain link:https://developer.enonic.com/docs/guillotine/next/api[functions for processing data^] before returning it.
Such URL functions return objects: `url` holds the URL as XP resolves it, while `path`, `queryString` and the other fields let you build your own.
In the Next.js app you will not write this selection by hand: the `imageUrlQuery()` helper from the adapter generates it with every field, and `imageUrl()` turns the result into an absolute URL to the media API.
+
