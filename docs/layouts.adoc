...
Layouts are essentially parts, with an extra trick up their sleeve. Regions. This means that layouts may host other components, and build rich page structures.

== Task: Add layout to Enonic

Adding layouts is more or less identical to pages and parts. Simply follow the steps below:

. Verify the Enonic app has **the `2-column` layout descriptor** at:
+
.src/main/resources/cms/layouts/2-column/2-column.yaml
[source,yaml]
----
kind: "Layout"
title: "2 columns"
...
+
When completed, as always *redeploy the Enonic app* if you had to change it (happens automatically if you started `enonic dev`).
...
----
+
NOTE: This is an example of using CSS modules. For more details visit the https://nextjs.org/docs/app/getting-started/css[Next.js style documentation]
+
...
After completing these steps, you should be able to add the layout to a page, and then add components inside the layout.
