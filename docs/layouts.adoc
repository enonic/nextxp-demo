...
+
When completed, as always *redeploy the Enonic app* (happens automatically if you started sandbox with `--dev` flag).
...
In the next chapter we will look into <<templates#, page templates>>. Page templates allow us to reuse a page definition across multiple content items.
