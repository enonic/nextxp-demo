...
The purpose of routes is to expose APIs and other endpoints in Enonic XP, like the `:8080/api` API root, to the internet.
...
== Task: Configure media URLs

Just like <<rendering-basics#, you did locally>>, XP must return absolute media URLs for images to render in the front-end — this time using the public API route.

**Add the following line** to the `com.enonic.xp.portal.cfg` https://developer.enonic.com/docs/xp/stable/deployment/config[configuration file^] of your environment, using the route URL from the previous task:

.com.enonic.xp.portal.cfg
[source,properties]
----
media.defaultBaseUrl=<route URL>
----

[.red]#TODO: Describe how to edit XP config files in the Enonic Cloud console#
...
* ENONIC_API: <enonic ingress URL>/com.enonic.app.guillotine:graphql
* ENONIC_ADMIN: <enonic admin URL> (optional, only if admin is on a different domain than the API)
...
. **Install the `Next.js Preview` app** `Solution` -> `Applications` -> `Install` -> Search for `Next.js Preview`, select it and click install
. **Add configuration**.
+
image:console-configure-app.png[title="Configure the Next.js Preview application in the solution console",width=769px]
+
...
+
NOTE: The app automatically adds the configured front-end origins to Content Studio's Content Security Policy, so the preview from your Vercel URL works without any Content Studio configuration.
Unlike your local sandbox, a production server allows only explicitly configured URLs - the `http://localhost:3000` default applies to dev mode only, so this configuration step is required.
+
NOTE: If you imported content from your local environment, the following two steps can be skipped
+
NOTE: The Next.js Preview app adds a preview widget that works without adding the app to the site.
However, adding it to the site lets you select a named configuration from the app config file.
+
. Optionally, **add the Next.js Preview app to the site** to select a named configuration.
