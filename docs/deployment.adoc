...
* ENONIC_API: <route URL from the previous task, ending with /api>
...
. **Install the `Next.XP` app** `Solution` -> `Applications` -> `Install` -> Search for `Next.XP`, select it and click install
. **Add configuration**.
+
...
+
NOTE: The Next.XP app adds a preview widget that works without adding the app to the site.
However, adding it to the site lets you select a named configuration from the app config file.
+
. Optionally, **add the Next.XP app to the site** to select a named configuration.
