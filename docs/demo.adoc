...
* ENONIC_API: <URL to the Enonic API root, e.g. https://<environment>/api>
* ENONIC_ADMIN: <enonic admin URL> (optional, only if admin is on a different domain than the API)
* ENONIC_MEDIA_CDN: <media URL> (optional, only if images and attachments are served from a different domain than the API)
...
. **No changes needed in Content Studio** because Next.js preview widget is already installed in the solution.
+
. We only need to instruct Next.js preview widget where to find the frontend server:
+
[.red]#TODO: Screenshot needs attention — Next.js Preview App#
+
.Go to Applications tab in Enonic Cloud and select Next.js preview app:
...
Content Studio will ask every widget if it supports preview, and if so, it will ask the widget to render a preview of the content.
When asked, the Next.js preview widget will do following sequence:

. send encrypted request to the configured Next.js front-end to fetch XP path to frontend URL mappings
. during that request it will also initialize the Next.js preview mode if necessary
. use those mappings to generate a frontend URL for a given content
. return it back to Content Studio for rendering it directly

Your edits will only be visible in Content Studio, until published.

When publishing changes, Next.js preview widget should automatically trigger revalidation of all pages in the front-end, and the changes will go live.
