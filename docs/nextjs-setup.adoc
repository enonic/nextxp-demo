...
<2> Proxy: validates Content Studio preview requests (encrypted `xp` blob, draft mode), rewrites URLs without a locale prefix to the internal default-locale route and redirects `/<default locale>/...` to `/...`
<3> Contains your implementation of CMS components
<4> Phrase files for multi-language support
<5> Global styles for the site
<6> Layout file for all pages under the locale/content path
<7> The main rendering file, catching all requests satisfying the pattern `/[locale]/[[...contentPath]]`
<8> Global layout file that wraps all pages and adds common structures (eg. `<Header>`, `<LocaleContextProvider>`, etc.)
<9> Error page handler
<10> Not found page handler
<11> Folder containing API routes
...
ENONIC_API=http://127.0.0.1:8080/api <2>
...
<5> Optional.
Admin domain used in the CORS header.
Defaults to `ENONIC_API` domain.
